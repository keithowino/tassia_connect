...
import { ShoppingCart, Bell, User, Menu, X, MapPin, ChevronDown } from 'lucide-react';
    ...
    <header className="sticky top-0 z-50 bg-white shadow-sm border-b border-gray-100">
      <div className="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between gap-3">
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm font-medium text-gray-600">
          <Link to="/discover" className="hover:text-orange-500 transition-colors">Discover</Link>
          <Link to="/community" className="hover:text-orange-500 transition-colors">Community</Link>
        ...
        <div className="flex items-center gap-2">
          ...
          {user ? (
              ...
              {userMenuOpen && (
                <div className="absolute right-0 top-12 w-52 bg-white rounded-xl shadow-lg border border-gray-100 py-1 z-50">
                  ...
                  {(profile?.role === 'business_owner' || profile?.role === 'admin') && (
                    <Link to="/dashboard" onClick={() => setUserMenuOpen(false)} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">Business Dashboard</Link>
                  )}
                  {profile?.role === 'admin' && (
                    <Link to="/admin" onClick={() => setUserMenuOpen(false)} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">Admin Panel</Link>
                  )}
            ...
            <Link to="/auth" className="bg-orange-500 text-white px-4 py-1.5 rounded-full text-sm font-semibold hover:bg-orange-600 transition-colors">
              Sign In
            </Link>
          )}
      ...
      {menuOpen && (
        <div className="md:hidden bg-white border-t border-gray-100 px-4 py-3 flex flex-col gap-2">
          <Link to="/discover" onClick={() => setMenuOpen(false)} className="py-2 text-gray-700 font-medium">Discover</Link>
          <Link to="/community" onClick={() => setMenuOpen(false)} className="py-2 text-gray-700 font-medium">Community</Link>
          <Link to="/orders" onClick={() => setMenuOpen(false)} className="py-2 text-gray-700 font-medium">Orders</Link>
        </div>
      )}
