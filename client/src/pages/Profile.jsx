...
import { Check, Edit2, Heart, LogOut, Phone, Store, X } from "lucide-react";
import LoadingSpinner from "../components/common/LoadingSpinner";
import BusinessCard from "../components/business/BusinessCard";

const Profile = () => {
	// const { user, profile, signOut, refreshProfile } = useAuth();
	const { profile, user, favHeart, dummyBusinesses } = useData();

	const [editForm, setEditForm] = useState({ full_name: "", phone: "" });
	const [saving, setSaving] = useState(false);
	const [editing, setEditing] = useState(false);
	const [activeTab, setActiveTab] = useState("favorites"); // (useState < "favorites") | ("businesses" > "favorites")
	const [loading, setLoading] = useState(true);
	...
	useEffect(() => {
		...
		try {
			...
			//   };
			//   fetchData();
			// shorter: setFavorites(favHeart.map(item => item.businesses));
			setFavorites(favHeart.map((fav) => fav.businesses).filter(Boolean));
			setMyBusinesses(
				dummyBusinesses.filter((f) => f.owner_id === user.id),
			);
		} catch (error) {
			console.error(error);
		} finally {
			...
			<section className="max-w-xl mx-auto px-4 py-4 space-y-4">
				...
				{loading ? (
					...
					<div>
						...
						{myBusinesses.length === 0 ? (
							<div className="text-center py-12 bg-white rounded-2xl border border-gray-100">
								<Store className="text-gray-300 mx-auto mb-3" />
								<p className="text-gray-500">
