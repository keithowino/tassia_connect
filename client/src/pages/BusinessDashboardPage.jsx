...
export default function BusinessDashboardPage() {
  ...
  const [tab, setTab] = useState<DashTab>(isNew ? 'settings' : 'overview');
  const [saving, setSaving] = useState(false);
  const [showProductForm, setShowProductForm] = useState(false);

  const [form, setForm] = useState({
    name: '', tagline: '', description: '', category_id: '',
    address: '', floor_unit: '', location_label: 'Tassia Complex',
    phone: '', whatsapp: '', email: '', website: '',
    opening_time: '08:00', closing_time: '20:00',
    delivery_available: false, delivery_fee: 0, min_order: 0,
    cover_image: '', open_days: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'],
  });

  const [productForm, setProductForm] = useState({
    name: '', description: '', price: 0, type: 'product' as 'product' | 'service',
  ...
  const toggleDay = (day: string) => setForm(prev => ({
    ...prev,
    open_days: prev.open_days.includes(day) ? prev.open_days.filter(d => d !== day) : [...prev.open_days, day],
  }));
...
const BusinessDashboardPage = () => {
	...
	useEffect(() => {
		...
		try {
			setCategories(data.dummyCategories); // dummy set action
	...
	return (
        ...
        <>
          <div className="grid grid-cols-3 gap-3">
            {[
              { icon: <Eye size={18} className="text-blue-500" />, label: 'Views', value: business.view_count },
              { icon: <Star size={18} className="text-amber-500" />, label: 'Avg Rating', value: business.average_rating > 0 ? business.average_rating.toFixed(1) : 'N/A' },
              { icon: <ShoppingCart size={18} className="text-green-500" />, label: 'Orders', value: orders.length },
            ].map(s => (
          ...
          <div className="flex gap-1 bg-gray-100 rounded-2xl p-1 overflow-x-auto">
            {(['overview', 'products', 'orders', 'settings'] as DashTab[]).map(t => (
              <button key={t} onClick={() => setTab(t)} className={`flex-1 py-2 rounded-xl text-xs font-semibold capitalize whitespace-nowrap transition-all ${tab === t ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500'}`}>
                {t}
              </button>
            ))}
          </div>
        ...
        <div className="space-y-3">
          <button onClick={() => setShowProductForm(!showProductForm)} className="w-full flex items-center justify-center gap-2 bg-orange-50 text-orange-600 border border-orange-200 py-2.5 rounded-2xl font-semibold text-sm hover:bg-orange-100 transition-colors">
          ...
          {showProductForm && (
            <div className="bg-white rounded-2xl border border-gray-100 p-4 space-y-3">
              <div className="flex items-center justify-between">
                <h3 className="font-bold text-gray-900 text-sm">New Item</h3>
                <button onClick={() => setShowProductForm(false)}><X size={16} className="text-gray-400" /></button>
              </div>
              <div className="flex gap-2">
                {(['product', 'service'] as const).map(t => (
                  <button key={t} type="button" onClick={() => setProductForm(p => ({ ...p, type: t }))}
                    ...
                    {t === 'product' ? <><Package size={13} /> Product</> : <><Wrench size={13} /> Service</>}
                  </button>
                ))}
              </div>
              {[
                ...
                <div key={f.key}>
                  ...
                  <input type={f.type} value={(productForm as Record<string, string | number | boolean>)[f.key] as string} onChange={e => setProductForm(p => ({ ...p, [f.key]: e.target.value }))}
              ...
              <button onClick={handleAddProduct} className="w-full bg-orange-500 text-white py-2.5 rounded-xl font-semibold text-sm hover:bg-orange-600 transition-colors">Add Item</button>
            </div>
          )}
          ...
          {products.length === 0 ? (
            <div className="text-center py-10 bg-white rounded-2xl border border-gray-100">
              ...
              <p className="text-gray-500 text-sm">No products or services yet</p>
            </div>
          ) : (
            products.map(product => (
              <div key={product.id} className="bg-white rounded-2xl border border-gray-100 p-4 flex items-center gap-3">
                {product.image_url && <img src={product.image_url} alt={product.name} className="w-12 h-12 rounded-lg object-cover shrink-0" />}
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-gray-900 text-sm">{product.name}</p>
                  <p className="text-orange-500 font-bold text-sm">KES {product.price.toLocaleString()}</p>
                  <span className="text-xs text-gray-400 capitalize">{product.type}</span>
                </div>
                <button onClick={() => deleteProduct(product.id)} className="p-2 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors">
                  <Trash2 size={15} />
                </button>
              </div>
            ))
				...
				{/* {tab === 'orders' && business && (
        <div className="space-y-3">
          <h2 className="font-bold text-gray-900 flex items-center gap-2"><ClipboardList size={18} className="text-orange-500" /> Incoming Orders</h2>
          {orders.length === 0 ? (
            <div className="text-center py-10 bg-white rounded-2xl border border-gray-100">
            ...
            orders.map(order => (
              <div key={order.id} className="bg-white rounded-2xl border border-gray-100 p-4">
                ...
                {order.order_items && <p className="text-sm text-gray-600 mb-2">{order.order_items.map(i => `${i.name} ×${i.quantity}`).join(', ')}</p>}
                {order.notes && <p className="text-xs text-gray-400 italic mb-2">"{order.notes}"</p>}
                <div className="flex items-center gap-2 flex-wrap">
                  {(['pending', 'accepted', 'preparing', 'ready', 'completed'] as Order['status'][]).map(s => (
                ...
                <p className="text-xs text-gray-400 mt-2 flex items-center gap-1">
                  <Clock size={11} /> {new Date(order.created_at).toLocaleString('en-KE')}
                </p>
              </div>
            ))
          )}
        ...
        <div className="space-y-4">
          {business.status === 'pending' && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-2xl p-4">
              <p className="text-yellow-800 font-semibold text-sm">Awaiting Approval</p>
              <p className="text-yellow-700 text-xs mt-1">Your business is under review. This usually takes 24-48 hours.</p>
            </div>
          )}
          <div className="bg-white rounded-2xl border border-gray-100 p-4 space-y-2">
            <h2 className="font-bold text-gray-900 mb-3">Quick Info</h2>
            <p className="text-sm text-gray-600"><span className="font-medium">Category:</span> {business.categories?.name || 'Uncategorized'}</p>
            <p className="text-sm text-gray-600"><span className="font-medium">Location:</span> {business.location_label}, {business.floor_unit}</p>
            <p className="text-sm text-gray-600"><span className="font-medium">Hours:</span> {business.opening_time} – {business.closing_time}</p>
            <p className="text-sm text-gray-600"><span className="font-medium">Products:</span> {products.length} listed</p>
          </div>
          <Link to={`/business/${business.slug}`} className="block w-full text-center bg-gray-900 text-white py-3 rounded-2xl font-semibold text-sm hover:bg-gray-800 transition-colors">
            View Public Profile
          </Link>
        </div>
      )} */}
