...
import { MapPin, Eye, EyeOff, Mail, Lock, User, Phone, Store } from 'lucide-react';
...
export default function AuthPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(false);
    navigate(tab === 'signup' && role === 'business_owner' ? '/dashboard/new' : '/');
  };
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <Link to="/" className="inline-flex items-center gap-2 mb-4">
            ...
            <span className="font-bold text-gray-900 text-xl">Tassia<span className="text-orange-500">Connect</span></span>
          </Link>
          <p className="text-gray-500 text-sm">{tab === 'signin' ? 'Welcome back to your community' : 'Join the Tassia community'}</p>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-lg p-6">
          ...
          {tab === 'signup' && (
            <div className="flex gap-2 mb-4">
              {(['user', 'business_owner'] as Role[]).map(r => (
                <button
                  key={r}
                  onClick={() => setRole(r)}
                  className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl border text-sm font-medium transition-all ${
                    role === r ? 'border-orange-400 bg-orange-50 text-orange-700' : 'border-gray-200 text-gray-600 hover:border-gray-300'
                  }`}
                >
                  {r === 'user' ? <><User size={15} /> Resident</> : <><Store size={15} /> Business Owner</>}
                </button>
              ))}
            </div>
          )}
          ...
          {tab === 'signin' && (
            <p className="text-center text-sm text-gray-500 mt-4">
              ...
              <button onClick={() => setTab('signup')} className="text-orange-500 font-semibold hover:text-orange-600">
                Sign up free
              </button>
            </p>
          )}
...
const Auth = () => {
	return (
		<>
			...
			<section className="auth">Auth</section>
		</>
	);
};
